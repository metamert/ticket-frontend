...
import Bus from "../../../components/Boxes/busBox"
...
function StickyHeadTable({ admin, history }) {
  ...
  const edit = async (data) => {
    try {
      ...
      myHeaders.append("jwt_token", admin.jwt_token);
  ...
  const Update = async () => {
    ...
    try {
      ...
      parseRes.data.map((item) => {
        const { id,  name,busType, wifi, foodService, tv } = item;
        
        arr.push(
          createData(
            id,
            name,busType, wifi?<CheckCircle/>:"-", foodService?<CheckCircle/>:"-", tv?<CheckCircle/>:"-",
